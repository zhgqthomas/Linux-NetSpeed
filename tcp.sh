...
sh_ver="1.3.3"
github="raw.githubusercontent.com/zhgqthomas/Linux-NetSpeed/master"
...
#安装BBRplus内核
installbbrplus(){
	kernel_version="5.10.78-bbrplus"
	if [[ "${release}" == "centos" ]]; then
		wget -N --no-check-certificate https://${github}/bbrplus/${release}/${version}/kernel-${kernel_version}.rpm
		yum install -y kernel-${kernel_version}.rpm
		rm -f kernel-${kernel_version}.rpm
		kernel_version="5.10.78_bbrplus" #fix a bug
	elif [[ "${release}" == "debian" || "${release}" == "ubuntu" ]]; then
...
check_status(){
	...
	if [[ ${kernel_version_full} = "5.10.78-bbrplus" ]]; then
